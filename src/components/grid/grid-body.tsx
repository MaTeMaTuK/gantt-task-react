...
export const GridBody: React.FC<GridBodyProps> = memo(
  ({
    ...
    const { gridRows, invalidColumn, rowLines, y } = useMemo(() => {
      let y = 0;
    ...
    }, [columnWidth, dates, taskListHeight, todayColor, viewMode, y]);
    const invalidBar = useMemo(
      () => (
        <g>
          <rect
            x={translateX}
      ...
      [columnWidth, invalidBarClick, parts, rowHeight, translateX, translateY]
    );
    return (
      <g
        className="gridBody"
